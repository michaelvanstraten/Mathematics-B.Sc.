...
r Sei $(x^((n)))_(n in NN)$ eine Cauchy-Folge in $ell^2(NN, RR)$, wobei
  ...
  Sei $epsilon > 0$. Da $(x^((n)))$ eine Cauchy-Folge ist, existiert $N in NN$
  ...
  Sei $epsilon > 0$ und $N$ wie oben gewählt. Für $n, m >= N$ und beliebiges $K in NN$ gilt:
  $
  ...
  Insbesondere ist $x^((n)) - x in ell^2(NN, RR)$ für $n >= N$. Da $x^((n)) in ell^2(NN, RR)$,
  folgt $x = x^((n)) - (x^((n)) - x) in ell^2(NN, RR)$.

  Somit konvergiert $(x^((n)))$ gegen $x$ in $ell^2(NN, RR)$, was die Vollständigkeit beweist.
...
+ Wir zeigen, dass $ell^2(NN, RR)$ nicht folgenkompakt ist, indem wir eine Folge
  ...
  Angenommen, $(e^((n_k)))_(k in NN)$ wäre eine konvergente Teilfolge mit Grenzwert
  $x in ell^2(NN, RR)$. Dann müsste für $epsilon = 1 < sqrt(2)/2$ ein $K in NN$
  existieren, so dass für alle $j, k >= K$ gilt:
  $
    norm(e^((n_j)) - e^((n_k)))_(ell^2) <= norm(e^((n_j)) - x)_(ell^2) + norm(x - e^((n_k)))_(ell^2) < 2 epsilon = 2.
  $

  Dies ist ein Widerspruch, da $norm(e^((n_j)) - e^((n_k)))_(ell^2) = sqrt(2) > 2$
  für $j != k$.
...
#exercise([20])[
  ...
  $f: K -> Y$ stetig. Beweisen Sie, mit Hilfe des Lemmas von der Lebesguezahl dass $f$ gleichmäßig
  stetig ist ohne den und schon bekannten Satz von Cantor–Heine zu benutzen.

  *Anfang des Beweises.* Sei $epsilon > 0$. Für jedes $x in X$ sei $delta_x > 0$,
  $
...
#exercise([20 = 10 + 10])[
  + Sei $K$ ein kompakter metrischer Raum, $Y$ ein metrischer Raum und $f: K -> Y$ stetig und
    bijektiv. Zeigen Sie, dass $f^(-1)$ stetig ist.

  + Finden Sie metrische Räume $X, Y$ und eine stetige bijektive Abbildung $f: X -> Y$, so dass
    $f^(-1)$ unstetig ist.
]

*Beweis*

+ Laut Proposition 8.12 ist $f^(-1)$ genau dann stetig, wenn für jede abgeschlossene
  Teilmenge $U subset.eq K$ gilt, dass $(f^(-1))^(-1)(U) = f(U)$ ebenfalls
  abgeschlossen ist.
  ...
  und stetige Bilder kompakter Mengen kompakt sind, folgt, dass $f(U)$ kompakt ist.
  Da kompakte Teilmengen metrischer Räume abgeschlossen sind, ist $f(U)$
  abgeschlossen in $Y$.
