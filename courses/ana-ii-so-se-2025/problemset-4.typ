...
#exercise([30 = 10 + 10 + 10])[
  Sei $f: RR -> RR$ eine Funktion. Sei $x_0 in RR$ und $(x_n) in RR^NN$
  (soweit möglich) rekursiv definiert durch
  $
  ...
  + Sei $f: RR -> RR$ zweimal stetig differenzierbar und $xi in RR$ eine
    ...
    Folgern Sie, dass das Newton-Verfahren für $x_0 in I$ gegen $xi$ konvergiert.
...
+ *Teil (a) und (b):* Sei $q := sup{abs(N'(x)) : x in I} < 1$ nach Voraussetzung.
  Da $N'$ stetig ist und $I$ kompakt, wird das Supremum angenommen.

  Für beliebige $x, y in I$ folgt aus dem Mittelwertsatz der Differentialrechnung:
  Es gibt ein $theta in [x, y]$ (bzw. $[y, x]$) mit
  $
  ...
  Für $N(I) subset.eq I$: Da $xi$ eine Nullstelle von $f$ ist, haben wir $f(xi) = 0$,
  also $N(xi) = xi - f(xi) / f'(xi) = xi - 0 / f'(xi) = xi$. Das heißt, $xi$ ist ein
  Fixpunkt von $N$.
...
+ Für $f(x) = x^3 - c$ haben wir $f'(x) = 3x^2$ und $xi = root(3, c)$ (da $f(xi) = 0$).
  ...
  Wir benötigen $abs(N'(x)) <= 1 / 2$ für alle $x in I = [root(3, c) - R, root(3, c) + R]$.
  ...
  Da $root(3, 4) - 1 approx 0.587$ und $1 - root(3, 4 / 7) approx 0.176$, ist das Minimum
  durch den zweiten Term gegeben:
  $
    R <= root(3, c)(1 - root(3, 4 / 7)).
  $

  Zusätzlich müssen wir sicherstellen, dass $f'(x) = 3x^2 != 0$ für alle $x in I$.
  Das ist erfüllt, solange $0 in.not I$, d.h. $root(3, c) - R > 0$, also $R < root(3, c)$.
